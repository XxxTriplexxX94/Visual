...
public aspect MSDCoordinatorAspect extends MSDCoordinator
{
	...
	public void Env.Wrappercall(Car _car,String String_symbolic_var_PickUp)
	{
		_car.call(String_symbolic_var_PickUp);
	}
	...
	public void Car.Wrapperdrive(Car _car)
	{
		_car.drive();
	}
	...
	private void afterImpl(MSDMethod envMeth) 
	{
		...
		MSDAspectLCS7.aspectOf().getCutState(monitoringEnabled, executingEnabled, coldViolation, hotViolation);
		aspects.add(MSDAspectLCS7.aspectOf());
	...
	protected void wrapperCall(MSDMethod MSDm)
	{
		...
		switch (MSDm.messageID)
		{
			...
			case MSDMethods.Car_Car_drive:
			 ((Car)MSDm.sourceInstance).Wrapperdrive(
				(Car)MSDm.targetInstance);
			break;
